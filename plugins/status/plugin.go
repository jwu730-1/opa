...
// BundleLoadDurationNanoseconds represents the configuration for the status.promtheus_config.bundle_loading_duration_ns settings
type BundleLoadDurationNanoseconds struct {
	Buckets []float64 `json:"buckets,omitempty"` // the float64 array of buckets representing nanoseconds or multiple of a nanoseconds
}
...
func (p *Plugin) reconfigure(config interface{}) {
	...
	if newConfig.Prometheus && !p.config.Prometheus {
		...
		p.collectors.ReregisterCustomizedCollectors(p.manager.PrometheusRegister(), newConfig.PrometheusConfig, p.logger)
	}
